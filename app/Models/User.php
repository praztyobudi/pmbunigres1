...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function getProgresAttribute(){
        ...
        else if(!empty($this->biodata) && is_null($this->pembayaranDaftarUlang())) return 'tes online';
        else if(!is_null($this->moodleAccount) && (is_null($this->moodleAccount->nilai_tpa) || $this->moodleAccount->nilai_tpa != 0)) return 'berkas';
        else if(!empty($this->biodata) && is_null($this->berkas)) return 'keluarga';
        else if(!empty($this->biodata) && is_null($this->ayah())) return 'biodata';
