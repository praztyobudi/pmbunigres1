...
class ProgramStudiController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validatedData = $this->validate($request, [
            'id' => 'nullable',
            'level' => 'required|exists:jenjang,id',
            'faculty' => 'exists:fakultas,id',
            'major' => 'required|string',
            'student_id_code' => 'required|string',
            'system_code' => 'required|string',
            'test_link' => 'required|string'
        ]);
        ...
        try {
            Prodi::updateOrCreate(
                ...
                [
                    'jenjang_id' => $validatedData['level'],
                    'fakultas_id' => $validatedData['faculty'],
                    'nama' => $validatedData['major'],
                    'kode_prodi_nim' => $validatedData['student_id_code'],
                    'kode_prodi_siakad' => $validatedData['system_code'],
                    'link_tpa' => $validatedData['test_link']
                ]);
            ...
            $res = [
                'status' => 'danger',
                'message' => 'Data program studi gagal dimasukkan. ERR_CODE: '.$e->getMessage()
            ];
        }
